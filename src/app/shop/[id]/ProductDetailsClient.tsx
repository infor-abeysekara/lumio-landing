...
import { 
  ...
  Truck, ShieldCheck, Headphones, RotateCcw,
            ...
            <div className="min-h-[200px]">
              {activeTab === 'Description' && (
                <div className="prose prose-blue max-w-none text-gray-600">
                  <p className="whitespace-pre-line leading-relaxed">{product.long_description || product.description}</p>
                </div>
              )}
              ...
              {relatedProducts.map((p: any, i: number) => (
                <div key={p.id} className="min-w-[280px] w-[280px] md:min-w-[300px] md:w-[300px] snap-start flex-shrink-0">
                  <motion.div
                    ...
                    className="bg-gray-50/50 hover:bg-white rounded-[2rem] border border-gray-100 p-5 md:p-6 flex flex-col hover:border-[#00a3ff]/30 transition-all duration-300 hover:shadow-2xl hover:shadow-[#00a3ff]/10 group cursor-pointer"
                    onClick={() => router.push(`/shop/${p.id}`)}
                  >
                    <div className="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 bg-[#00a3ff]/5 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                    ...
                    <div className="flex-1 flex flex-col relative z-10">
                      ...
                      <div className="mt-auto pt-4 flex items-center justify-between">
                        <span className="font-black text-xl text-[#00a3ff] tracking-tight">
                          <span className="text-xs text-gray-400 font-bold mr-1">Rs.</span>
                          {Number(p.price).toLocaleString()}
                        </span>
                        <div className="text-[#00a3ff] border border-[#00a3ff]/20 rounded-full p-2 group-hover:bg-[#00a3ff] group-hover:text-white transition-colors">
